...
import XCTest
import Foundation
@testable import bsdinstruments

/// Subprocess-based end-to-end tests that exec the actual `bsdinstruments`
/// binary built by `swift build` and validate behavior.
///
/// Tests are split into two tiers:
///
///   1. **No-root tier** — `bsdinstruments list` and `bsdinstruments generate` don't open
///      a libdtrace handle, so they run in any context. These run on
///      every `swift test` invocation.
///
///   2. **Root tier** — `bsdinstruments watch` and `bsdinstruments probes` need libdtrace
...
final class IntegrationTests: XCTestCase {
    ...
    /// Path to the bsdinstruments binary that was built alongside this test
    /// bundle. The test bundle and the executable are siblings inside
    /// the SwiftPM build directory's `<triple>/debug/` directory:
    ///
    ///   <build-path>/x86_64-unknown-freebsd/debug/bsdinstrumentsPackageTests.xctest   ← test bundle
    ///   <build-path>/x86_64-unknown-freebsd/debug/bsdinstruments                      ← executable
    ///
    ...
    /// Run the bsdinstruments binary with the given args, capture stdout +
    /// stderr + exit status. Returns nil if the binary doesn't exist.
    ///
    /// Enforces a per-subprocess timeout (default 10 seconds). If the
    /// process is still running when the timeout expires, bsdinstruments gets
    ...
    ///
    /// **Pipe draining is critical.** bsdinstruments's text-mode profiles (and
    ...
    private func runBSDInstruments(
        _ args: [String],
        asRoot: Bool = false,
        ...
        let bin = bsdinstrumentsBinaryPath
        ...
        do {
            ...
            XCTFail("failed to launch bsdinstruments: \(error)")
            return nil
        }
    ...
    private func skipIfBinaryMissing() throws {
        guard FileManager.default.fileExists(atPath: bsdinstrumentsBinaryPath) else {
            throw XCTSkip("bsdinstruments binary not found at \(bsdinstrumentsBinaryPath); run `swift build` first")
        }
    }
    ...
    func testListSubcommandRunsCleanly() throws {
        ...
        guard let result = runBSDInstruments(["list"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
        XCTAssertEqual(result.exitCode, 0,
            "bsdinstruments list should exit 0, got \(result.exitCode); stderr: \(result.stderr)")
        XCTAssertFalse(result.stdout.isEmpty,
            "bsdinstruments list should produce output")
    }
    ...
    func testListSubcommandShowsAtLeast85Profiles() throws {
        ...
        guard let summary = lines.last(where: { !$0.isEmpty }) else {
            XCTFail("no summary line in bsdinstruments list output")
            return
        }
        ...
        XCTAssertGreaterThanOrEqual(count, 85,
            "bsdinstruments list reports \(count) profiles, expected ≥ 85 (dwatch parity)")
    }
    ...
    func testListSubcommandShowsKillProfile() throws {
        ...
        XCTAssertTrue(result.stdout.contains("kill"),
            "bsdinstruments list output should contain the 'kill' profile")
    }
    ...
    func testGenerateSubcommandRendersKill() throws {
        try skipIfBinaryMissing()
        guard let result = runBSDInstruments(["generate", "kill"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
        XCTAssertEqual(result.exitCode, 0,
            "bsdinstruments generate kill should exit 0; stderr: \(result.stderr)")
    ...
    func testGenerateSubcommandInjectsFilterPredicate() throws {
        try skipIfBinaryMissing()
        guard let result = runBSDInstruments(["generate", "kill", "--execname", "nginx"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
        XCTAssertEqual(result.exitCode, 0)
    ...
    func testGenerateSubcommandInjectsDurationTick() throws {
        try skipIfBinaryMissing()
        guard let result = runBSDInstruments(["generate", "kill", "--duration", "5"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
        XCTAssertEqual(result.exitCode, 0)
    ...
    func testGenerateSubcommandInjectsStackActions() throws {
        try skipIfBinaryMissing()
        guard let result = runBSDInstruments(["generate", "kill", "--with-stack", "--with-ustack"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
        XCTAssertEqual(result.exitCode, 0)
    ...
    func testGenerateSubcommandRefusesUnknownProfile() throws {
        try skipIfBinaryMissing()
        guard let result = runBSDInstruments(["generate", "definitely-not-a-real-profile"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
    ...
    func testGenerateSubcommandRequiresKinstParams() throws {
        ...
        guard let result = runBSDInstruments(["generate", "kinst"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
        XCTAssertNotEqual(result.exitCode, 0,
    ...
    func testGenerateSubcommandWithKinstParams() throws {
        try skipIfBinaryMissing()
        guard let result = runBSDInstruments([
            "generate", "kinst",
            "--param", "func=vm_fault",
            "--param", "offset=4",
        ]) else {
    ...
    ///
    /// Iterates every bundled profile, runs `bsdinstruments watch <profile>
    ...
    ///
    /// Per-profile timeout is 8 seconds (the runBSDInstruments helper's
    ...
    func testEveryBundledProfileCompilesViaLibdtrace() throws {
        ...
        for profile in ProfileLoader().all() {
            ...
            guard let result = runBSDInstruments(args, timeout: 8.0) else {
                XCTFail("could not run bsdinstruments")
                return
            }
    ...
    func testJsonFormatProducesValidJSONL() throws {
        ...
        XCTAssertEqual(result.exitCode, 0,
            "bsdinstruments watch --format json should exit 0; stderr: \(result.stderr)")
        XCTAssertFalse(result.stdout.isEmpty,
    ...
    /// Smoke test for `--format otel`: verify bsdinstruments exits cleanly
    ...
    func testOtelFormatExitsCleanly() throws {
        ...
        XCTAssertEqual(result.exitCode, 0,
            "bsdinstruments watch --format otel should exit 0; stderr: \(result.stderr)")
    ...
    func testOtelFormatWithAggregationProfile() throws {
        ...
        XCTAssertEqual(result.exitCode, 0,
            "bsdinstruments watch syscall-counts --format otel should exit 0; stderr: \(result.stderr)")
    }

    // MARK: - Root tier: probes subcommand

    func testProbesSubcommandLightSmokeTest() throws {
        try XCTSkipUnless(isRoot, "bsdinstruments probes needs root")
        try skipIfBinaryMissing()
        guard let result = runBSDInstruments(["probes", "--provider", "proc"]) else {
            XCTFail("could not run bsdinstruments")
            return
        }
        XCTAssertEqual(result.exitCode, 0,
            "bsdinstruments probes --provider proc should succeed; stderr: \(result.stderr)")
        XCTAssertTrue(result.stdout.contains("proc:"),
            "should list at least one proc probe")
    }
}

/// Locked byte accumulator used by `runBSDInstruments`'s background pipe drain.
