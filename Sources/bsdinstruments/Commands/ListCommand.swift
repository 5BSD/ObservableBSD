...
// MARK: - bsdinstruments list

/// `bsdinstruments list` — print every profile the loader found, across all
/// three sources (bundled, system, user).
struct ListCommand: ParsableCommand {

    static let configuration = CommandConfiguration(
        commandName: "list",
        abstract: "List every profile bsdinstruments knows about.",
        discussion: """
            Profiles are loaded from three sources at startup: \
            bundled SwiftPM resources inside the bsdinstruments binary, \
            /usr/local/share/bsdinstruments/profiles/ (where the FreeBSD port \
            drops the catalog), and ~/.bsdinstruments/profiles/ (per-user). \
