...
/// predicate and bsdinstruments leaves the loaded `.d` file's clauses
/// untouched. When at least one flag is set, bsdinstruments composes a
/// D predicate string and substitutes it at the
/// `/* @bsdinstruments-predicate */` marker in the loaded script.
///
...
struct FilterOptions: ParsableArguments {
    ...
    /// for substitution at the **`@bsdinstruments-predicate`** marker.
    ///
    ...
    /// suitable for substitution at the **`@bsdinstruments-predicate-and`**
    ...
    @Option(
        ...
        help: ArgumentHelp(
            ...
            discussion: """
                ...
                open-ended profiles. bsdinstruments injects an equivalent \
...
///
/// bsdinstruments injects `stack();` / `ustack();` actions at the
/// `/* @bsdinstruments-stack */` / `/* @bsdinstruments-ustack */` markers in the loaded
    ...
    @Option(
        ...
        help: ArgumentHelp(
            ...
            discussion: """
                ...
                (default 128m). bsdinstruments defaults to 4m for text output \
    ...
    @Option(
        ...
        help: ArgumentHelp(
            ...
            discussion: """
                ...
                bsdinstruments can read them. Lower values drain faster (fewer \
                drops) but cost more user/kernel transitions. Also \
                sets the maximum latency between an in-script exit(0) \
                and bsdinstruments noticing. Default: 50ms.
                """
        )
    )
