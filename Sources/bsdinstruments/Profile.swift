...
/// `/* @bsdinstruments-predicate */` marker where bsdinstruments injects CLI filter
...
struct Profile: Sendable {
    ...
    /// Where this profile came from. Used for `bsdinstruments list` display
    ...
    func render(
        ...
        //    `/* @bsdinstruments-predicate */` and bsdinstruments replaces it with the
        //    full `/clause/` block. Profiles WITH their own
        //    predicate use `/* @bsdinstruments-predicate-and */` inside the
        //    existing predicate block, and bsdinstruments replaces it with
        //    `&& clause` (or empty if no filter is set). If the
        //    marker is absent, bsdinstruments does nothing — profiles without
        //    either marker silently ignore filter flags.
        rendered = rendered.replacingOccurrences(
            of: "/* @bsdinstruments-predicate-and */",
            with: predicateAnd
        )
        rendered = rendered.replacingOccurrences(
            of: "/* @bsdinstruments-predicate */",
            with: predicate
        )

        // 3. Stack capture injection.
        //    Replace `/* @bsdinstruments-stack */` markers with `stack();` if
        //    --with-stack was passed, otherwise empty. Same for
        //    `/* @bsdinstruments-ustack */` and --with-ustack. Profiles
        //    without the markers silently don't get stacks even if
        //    the flag is set — they simply don't opt in.
        rendered = rendered.replacingOccurrences(
            of: "/* @bsdinstruments-stack */",
        ...
        if withUstack {
            if withStack {
                ustackAction = "printf(\"__BSDINSTRUMENTS_USTACK__\\n\"); ustack();"
            } else {
                ustackAction = "ustack();"
            }
        } else {
...
enum ProfileOrigin: Int, Sendable, Comparable {
    ...
    case systemDir = 1     // /usr/local/share/bsdinstruments/profiles/
    case userDir = 2       // ~/.bsdinstruments/profiles/
...
enum ProfileError: Error, CustomStringConvertible {
    ...
    /// The user asked for a profile name bsdinstruments doesn't have.
    ...
    var description: String {
        ...
        case .unknownProfile(let name):
            return "unknown profile '\(name)'. Try `bsdinstruments list`."
        }
    }
}
