...
final class BundledProfilesTests: XCTestCase {
    ...
    func testBundledCatalogShipsAtLeast85Profiles() {
        // dwatch ships 85; bsdinstruments should hit at least parity.
    ...
    /// throw and that no `@bsdinstruments-*` marker token survives.
    func testEveryBundledProfileRendersAndConsumesAllMarkers() throws {
        for profile in loader().all() {
            do {
                ...
                XCTAssertFalse(
                    rendered.contains("@bsdinstruments-"),
                    "profile '\(profile.name)' has a leftover @bsdinstruments-* marker after rendering — token wasn't consumed"
                )
                XCTAssertFalse(
                    rendered.isEmpty,
    ...
    func testEveryBundledProfileRendersWithAllOptionsOn() throws {
        for profile in loader().all() {
            do {
                ...
                XCTAssertFalse(rendered.contains("@bsdinstruments-"),
                    "profile '\(profile.name)' leaked an @bsdinstruments-* marker with all options on")
    ...
    /// canonical 85-profile baseline) must exist in the bsdinstruments bundled
    ...
    func testEveryDwatchProfileNameIsCovered() {
        ...
        let bsdinstrumentsNames = Set(loader.all().map { $0.name })
        let missing = dwatchProfiles.subtracting(bsdinstrumentsNames).sorted()
        XCTAssertTrue(missing.isEmpty,
            "bsdinstruments is missing \(missing.count) dwatch parity profile(s): \(missing)")
    }
