...
struct WatchRunner {
    ...
    func run() throws {
        ...
        do {
            ...
            throw BSDInstrumentsRunError.compileFailed(
    ...
    private func runTextBackend(handle: borrowing DTraceHandle, stopFlag: StopFlag) throws {
        ...
        loop: while !stopFlag.isSet {
            ...
            case .error:
                FileHandle.standardError.write(Data(
                    "bsdinstruments: libdtrace work() failed: \(handle.lastErrorMessage)\n".utf8
                ))
                break loop
            }
        }
        ...
        do {
            ...
            FileHandle.standardError.write(Data(
                "bsdinstruments: aggregation snapshot failed: \(error)\n".utf8
            ))
        }
    ...
    private func runStructuredBackend(handle: borrowing DTraceHandle, stopFlag: StopFlag, aggKeyNames: [String: [String]]) throws {
        ...
        try handle.onDrop { [exporterRef = exporter] drop in
            fputs(
                "bsdinstruments: dropped \(drop.drops) record(s) (\(drop.message))\n",
                stderr
            )
            // Report drops to the OTLP exporter so the next batch
            // includes a bsdinstruments.drops attribute.
        ...
        try handle.onBufferedOutput { data in
            ...
            for line in lines {
                ...
                } else if trimmed == "__BSDINSTRUMENTS_USTACK__" {
    ...
    /// Map DTraceCore's AggregationAction to bsdinstruments's AggregationKind.
...
enum BSDInstrumentsRunError: Error, CustomStringConvertible {
