...
// MARK: - bsdinstruments watch

/// `bsdinstruments watch` — load a profile (by name from the catalog or by
...
struct WatchCommand: ParsableCommand {
    ...
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            Pass a profile name from `bsdinstruments list`, or `-f /path/to/script.d` \
            for an arbitrary D file. Filter flags (--pid, --execname, etc.) \
            are injected at the profile's `/* @bsdinstruments-predicate */` marker if \
    ...
    @Argument(
        help: ArgumentHelp(
            ...
            discussion: "Use `bsdinstruments list` to see every available profile. For an arbitrary .d file, use `-f`."
        )
    )
    var profile: String?
    ...
    func run() throws {
        ...
        let resource = ResourceAttributes(
            serviceName: otelEnv.serviceName ?? "bsdinstruments",
