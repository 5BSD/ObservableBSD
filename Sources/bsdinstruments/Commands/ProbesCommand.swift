...
// MARK: - bsdinstruments probes

/// `bsdinstruments probes` — list every DTrace probe (built-in or USDT) the
    ...
    func run() throws {
        ...
        do {
            ...
            FileHandle.standardError.write(Data(
                "bsdinstruments probes: failed to open libdtrace (\(error)). Are you root?\n".utf8
            ))
        ...
        do {
            ...
            FileHandle.standardError.write(Data(
                "bsdinstruments probes: listProbes failed: \(error)\n".utf8
            ))
            throw ExitCode.failure
        }
